...
def attempt_pca(input_dataframe):
    data_assembler = VectorAssembler(
        inputCols=['w_good_mental_health', 'w_poor_mental_health', 'w_extremely_stressful', 'w_mood_disorder',
                   'w_high_belonging', 'w_high_life_satisftn'], outputCol="features")
...
def get_mh_score(input_dataframe, year):
    ...
    results = input_dataframe.withColumn('w_good_mental_health',
        ...
        'w_poor_mental_health', input_dataframe.poor_mental_health * matrix[5][1]).withColumn('w_extremely_stressful',
                                                                                              input_dataframe.extremely_stressful *
                                                                                              matrix[5][2]).withColumn(
        'w_mood_disorder', input_dataframe.mood_disorder * matrix[5][3]).withColumn('w_high_belonging',
                                                                                    input_dataframe.high_belonging *
                                                                                    matrix[5][4]).withColumn(
        'w_high_life_satisftn', input_dataframe.high_life_satisftn)
...
if __name__ == '__main__':
    ...
    spark = SparkSession.builder.appName('Mental Health Weighted PCA').getOrCreate()
